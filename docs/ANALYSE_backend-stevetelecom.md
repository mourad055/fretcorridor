...
1. **`RealGeoAdapter` neutralisé, pas supprimé.** Le point §2 n'a pas encore été tranché en équipe — plutôt que de bloquer indéfiniment un commit par ailleurs solide, `RealGeoAdapter` et `MockGeoAdapter` ont eu leurs annotations `@Profile` inversées : `MockGeoAdapter` (`@Profile("!real-geo")`) redevient l'implémentation active par défaut, `RealGeoAdapter` (`@Profile("real-geo")`) devient opt-in, activable via `-Dspring.profiles.active=real-geo` une fois la décision prise. Le code de `RealGeoAdapter` est conservé intact — rien à refaire le jour où l'équipe tranche. **Suite de tests gateway rejouée après ce correctif : 94/94 verts**, y compris `AxeControllerIsolationTest`.
