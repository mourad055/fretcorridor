...
- j;ai le clone du repo en local donc on travaille en local avant de posser en ligne sur git ; j'utlise ouvent des pipelines ci/cd avec actions github mais pour boussole fret IA tu vas m'aider a hberger sue versel et implemente cet pipeline ci/cd de tel sort eque a chaque push ca pousse egament en production ; donc du me donne le code  sur le terminal  bash (avec cat , pythonn3 , grep suivi de wc , pour eviter les erruers de syntaxe et le herodoc ) je code sur le terminal 
- Cloner le repo, tester que ça fonctionne, avant d'envoyer le code.(juste pour voir ce qui:'l fait encore faire ; je dis bien pas tput le temps pour econimer les tokens  si tu veux voir le code tu me donne des commandes cat grep ou puthon pour que je puisse de donner)
...
- **Windows (git bash)** : utiliser `git apply` ; **ne pas utiliser `python3`**, préférer `python` ou `sed` suivi de `grep` pour insérer du code dans de gros fichiers. sur windows pas necessaire sur ubuntu 
