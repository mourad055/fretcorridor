...
### 1.6 Suivi temps réel Bureau branché sur des données réelles (EF-TRK-04, ENF-MUL-01, RG-043)

Le point n°4 de `docs/ROADMAP_INTEGRATION_gateway.md` (positions véhicules) est débloqué le 2026-08-10 (cf. [ADR 0014](adr/0014-bur-consomme-position-eta.md)) : `service-bur` consomme l'événement Kafka `position-eta` (déjà publié par `service-trk`/Moteur pour EF-TRK-01/02/03) et matérialise une vue Bureau, exposée au gateway via `ServiceBurPositionAdapter` (remplace `MockTrkAdapter`).

- **EF-TRK-04** (« afficher toute position avec son horodatage et son âge ») : `PositionResponse.from` (gateway) calcule `ageSecondes` à chaque restitution, jamais stocké — inchangé par ce travail, mais désormais calculé sur un `capturedLe` réel (issu de l'événement Kafka) plutôt que simulé par le mock.
  → [`PositionResponse.java`](../backend/gateway/src/main/java/com/fretcorridor/gateway/infrastructure/rest/trk/dto/PositionResponse.java)
- **ENF-MUL-01** (isolation multi-tenant) : `PositionControllerIsolationTest` continue de vérifier le contrat (Bureau A ne voit jamais les positions de Bureau B) via la fixture `MockTrkAdapter` (déplacée en `src/test/java`, `@Primary`, même mécanisme que GEO) ; l'isolation réelle est vérifiée côté `service-bur` par `PositionRepositoryAdapterIntegrationTest.does_not_leak_positions_across_tenants`.
- **Aucun changement requis côté Angular** : `PositionService.list()` (web) appelle déjà `GET /api/v1/bureau/positions`, un endpoint gateway préexistant qui ne connaissait jusqu'ici que `TrkPort` — remplacer l'implémentation du port suffit, aucune ligne de frontend à modifier pour que l'écran affiche désormais de vraies positions.
- Contrat `shared-contracts/openapi/gateway-bureau-positions.yaml` toujours respecté à l'octet (mêmes 6 champs) ; sa description, qui mentionnait encore un mock en attente de Moteur, a été corrigée pour refléter l'architecture réelle (service-bur, pas service-trk directement).
- **Écart mineur, assumé et documenté (pas un manquement)** : `position-eta` porte aussi l'ETA calculé par Moteur pour EF-TRK-02 (`etaCentral`/`etaBorneBasse`/`etaBorneHaute`, intervalle de confiance) ; `PositionEtaListener` ne retient que la position brute et ignore ces champs. Le CDC n'exige pas explicitement que le Bureau affiche cet ETA (EF-BUR-01 porte sur la supervision des *missions*, pas sur l'ETA par véhicule) — seul EF-TRK-04 (position + âge) est couvert ici. À réévaluer si le Bureau demande un jour à voir l'ETA sur son écran de suivi.

### 1.7 Déviations documentées plutôt que silencieuses
...
Les deux écarts Phase 1 identifiés (EF-BUR-02, EF-PAY-05) ont été corrigés le 2026-08-08, avec tests à l'appui (backend et web) — voir §2.1 et §2.2. Le seul point restant est mineur et documenté comme un choix assumé (§2.3, seuil d'agrégation hors base). Le 2026-08-10, le suivi temps réel Bureau (EF-TRK-04) est passé de données mockées à des données réelles (`service-bur` ← Kafka `position-eta`), sans aucune régression ni changement de contrat — voir §1.6. En l'état, `feature/web-socle` respecte le CDC v4 et le Plan d'Exécution V4.2 sur l'ensemble du périmètre vérifié dans ce document.
