-- Données de démonstration pour service-opt — chargées UNIQUEMENT sous le
-- profil "dev". Ce seed donne un barème tarifaire opérationnel sur les axes
-- de démonstration, afin que le moteur L4 ne tombe pas en mode dégradé
-- (prix null) à la publication de la première proposition.
--
-- Conformément au CDC §8.9 / EF-GEO-02 : le barème est configurable par axe
-- et type de véhicule, avec repli au défaut global si nécessaire.
--
-- Les valeurs ci-dessous sont des données de démo uniquement, pas des valeurs
-- de production. Elles doivent être remplacées par les barèmes métier réels
-- en exploitation. Idempotent : ON CONFLICT DO NOTHING.

SET search_path TO opt, public;

INSERT INTO bareme_tarification (
...
INSERT INTO composant_cout_service (id, bareme_id, code_service, montant)
VALUES
