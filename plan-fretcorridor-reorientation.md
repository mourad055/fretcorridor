...
- Deux contrats Kafka **Mobile → OPT** formalisés dans `shared-contracts/asyncapi/events/`
  (AsyncAPI 3.0.0), **publiés par service-cap** (Mobile — porteur des capacités et
  des actions du chauffeur, conforme CDC EF-CAP-*, déjà émetteur de `capacite-declaree` ;
  "service-mkt" dans une version antérieure était un choix de nommage erroné, corrigé) :
...
- **UC-MAT-02, modèle CDC strict (#140) — À FERMER**, superseded par §1. L'écran
  ...
  câblage data doit être refait pour appeler service-cap (Kafka) au lieu de la
...
| Écran swipe des demandes | **Mobile** | `propositions_mission_screen.dart` actuel liste les propositions avec boutons Accepter/Refuser en colonne — sous diffusion-course, un chauffeur peut recevoir plusieurs propositions simultanées pour des demandes différentes (diffusion à tous les compatibles), donc une pile swipeable (type `Dismissible`/`CardSwiper`) est plus adaptée qu'une liste boutonnée. Le provider (`proposition_mission_provider.dart`) devra de toute façon être réécrit pour consommer service-cap au lieu de la gateway REST (§1) — même chantier. | ⏳ À faire |
